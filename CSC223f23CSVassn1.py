...
#
# Student Notes:
#   In the first documentary commentary I filled in the gaps while simultaneously
#   adjusting various functions to meet the requirements of the assignment.
#   A key modification involved the transformation of the getStats() function  
#   to create different lists for each statistical distribution as per
#   the provided instructions.
#   I also ensured each table was created using their correct generator.
#   Then, I took all the student-made lists and put them into one table, inserting
#   headers at the beggining of the new table. Combining the student-made table with
#   the hand-out table.
#   Next, I modified readStartingTable, so that instead of an explicit line parser,
#   it used csv.reader.
#   Finally, I replaced the handout loop in STUDENT 5 with a
#   csv.writer object to write out the rows of data to the table and file.
#
...
def generateDistributionTable(seed, statsfilename, startingTable,
 ...
 #   listA = [int(generatorA.uniform(0, 94))
  #      for i in range(0,numberOfRows)]
   # getstats('RndUniform, seed = ' + str(seed),
    ...
    listC = [int(generatorA.normalvariate(50, 10))
             for i in range(0,numberOfRows)]
    getstats('RndNormal10, seed = ' + str(seed),
             listC, statsfileh)
   
    listD = list(map(lambda value : int(value), generatorB.normal(50, 10, numberOfRows)))
    getstats('NPNormal10, seed = ' + str(seed),
             listD, statsfileh)
   
    listE = [int(generatorA.normalvariate(50, 20)) for i in range(0,numberOfRows)]
    getstats("RndNormal20, seed = " + str(seed), listE, statsfileh)


    listF = list(map(lambda value : int(value),
                     generatorB.normal(50, 20, numberOfRows)))
    getstats('NPNormal20, seed = ' + str(seed),
             listF, statsfileh)
   
    listG = [int(generatorA.expovariate(0.1)) for i in range(0,numberOfRows)]
    getstats('RndExponent10, seed = ' + str(seed),
             listG, statsfileh)
   
    listH = list(map(lambda value : int(value), generatorB.exponential(10, numberOfRows)))
    getstats('NPExponent10, seed = ' + str(seed), listH, statsfileh)


    listI = [int(generatorA.expovariate(0.05)) for i in range(0, numberOfRows)]
    getstats('RndExponent20, seed = ' + str(seed), listI, statsfileh)


    listJ = list(map(lambda value : int (value), generatorB.exponential(20, numberOfRows)))
    getstats('NPExponent20, seed = ' + str(seed), listJ, statsfileh)
    ...
    for i in listJ:
        #listK.append(log2(i)+1)
        listK.append(round(log2(i+min(listJ)+1), 6))


    getstats('NPExp20Log2, seed = ' + str(seed), listK, statsfileh)
...
# Out[10]: [[1, 2, 3, 111, 222, 333], [11, 22, 33, -1, -2, -3]]
    ...
    #table1 = list(zip(listA, listB, listC, listD, listE, listF, listG, listH, listI, listJ, listK))    # zip also returns a generator
    table1 = list(zip(listC, listD, listE, listF, listG, listH, listI, listJ, listK))
    #table1.insert(0, ('RndUniform', 'NPUniform', 'RndNormal10', 'NPNormal10', 'RndNormal20', 'NPNormal20', 'RndExponent10', 'NPExponent10', 'RndExponent20', 'NPExponent20', 'NPExp20Log2')) # headings for the columns
    table1.insert(0, ('RndNormal10', 'NPNormal10', 'RndNormal20', 'NPNormal20', 'RndExponent10', 'NPExponent10', 'RndExponent20', 'NPExponent20', 'NPExp20Log2'))
    result = []
    for index in range(0, len(table1)):
        result.append(list(startingTable[index] + list(table1[index])))
...
def readStartingTable(startingCSVdatafile):
    ...
    incsv = csv.reader(datafile)
    table = [row for row in incsv]
...
if __name__ == '__main__':      # Entry code outside of any function.
    ...
    outcsv = csv.writer(outcsvfile, delimiter=',', quotechar='"')
    #outcsvhdr = ['RndUniform', 'NPUniform', 'RndNormal10', 'NPNormal10', 'RndNormal20', 'NPNormal20', 'RndExponent10', 'NPExponent10', 'RndExponent20', 'NPExponent20', 'NPExp20Log2']
    #outcsv.writerow(outcsvhdr)
